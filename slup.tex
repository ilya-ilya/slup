...
\begin{solution}
  ...
  \begin{equation*}
    \text{cov}(\xi_0(-1)^{N(t)},\xi_0(-1)^{N(s)}) = \mathsf{E}\left[\xi_0^2 (-1)^{N(t)+N(s)}\right] - \mathsf{E}\left[\xi_0(-1)^{N(t)} \right]\mathsf{E} \left[\xi_0(-1)^{N(s)}\right]
  \end{equation*}
   Вспомним, что для независимых случайных величин $\xi$ и $\eta$ выполнено $\mathsf{E}\xi\eta = \mathsf{E}\xi\mathsf{E}\eta$ и продолжим рассуждения:
    \begin{equation*}
      =   \mathsf{E}\eta^2 \mathsf{E}(-1)^{N(t)+N(s)} - \left(\mathsf{E}\eta\right)^2 \mathsf{E} \left[(-1)^{N(t)}\right] \mathsf{E} \left[ (-1)^{N(s)} \right]
    \end{equation*}
    Обозначим за $\psi(z) = e^{\mu(z-1)}$ - производящуюю функцию $Poiss(\mu)$. Заметим, что $\mathsf{E}\eta^2 \equiv 1$, $\mathsf{E}\eta = 0$ и $(-1)^{N(t)-N(s)} = (-1)^{N(t)+N(s)}$. Подставим в полученное выше выражение, продолжив рассуждения:
    \begin{equation*}
      = \mathsf{E}(-1)^{N(t)-N(s)} = \psi(-1) = e^{-2\lambda(t-s)},
    \end{equation*}
