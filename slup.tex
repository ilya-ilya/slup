...
\begin{solution}
  ...
  \end{equation*}
   Вспомним, что для независимых случайных величин $\xi$ и $\eta$ выполнено $\Expect\xi\eta = \Expect\xi\Expect\eta$ и продолжим рассуждения:
    ...
    \end{equation*}
    Обозначим за $\psi(z) = e^{\mu(z-1)}$ - производящуюю функцию $\Pois(\mu)$. Заметим, что $\mathsf{E}\eta^2 \equiv 1$, $\mathsf{E}\eta = 0$ и $(-1)^{N(t)-N(s)} = (-1)^{N(t)+N(s)}$. Подставим в полученное выше выражение, продолжив рассуждения:
    \begin{equation*}
