...
\begin{solution}
  ...
  \end{equation*}
   Вспомним, что для независимых случайных величин $\xi$ и $\eta$ выполнено $\Expect\xi\eta = \Expect\xi\Expect\eta$ и продолжим рассуждения:
    \begin{equation*}
      =   \Expect\xi_0^2 \Expect(-1)^{N(t)+N(s)} - \left(\Expect\xi_0\right)^2 \Expect\left[(-1)^{N(t)}\right] \Expect \left[ (-1)^{N(s)} \right]
    \end{equation*}
    Обозначим за $\psi_\mu(z) = e^{\mu(z-1)}$ - производящуюю функцию $\Pois(\mu)$. Заметим, что $\Expect\xi_0^2 \equiv 1$, $\Expect\xi_0 = 0$ и $(-1)^{N(t)-N(s)} = (-1)^{N(t)+N(s)}$. Подставим все "замеченное" в полученное выше выражение:
    \begin{equation*}
